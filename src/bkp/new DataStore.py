...
class DataStore (object):
    ...
    def __init__(self, ID, size = 1000, bf_hash_count = 5, miss_rate_alpha = 0.1, miss_rate_window = 1000, miss_rate_init = 0.5,
        ...
        self.FN_mr_alpha            = self.mr_alpha
        self.mr_win                 = miss_rate_window
        self.FN_mr_win              = self.mr_win       # Window size for speculative accesses (aaccesses to the cache in spite of negative ind') 
    ...
    def update_FN_mr(self):
        ...
        """
        self.FN_mr_win_estimate.append(float(self.FN_mr_win_miss_cnt[-1]) / self.FN_mr_win)
        self.FN_mr_cur.append (self.FN_mr_alpha * (self.FN_mr_win_estimate[-1]) + (1 - self.FN_mr_alpha) * self.FN_mr_cur[-1] )
    ...
    def access(self, key, is_speculative_accs = False):
        ...
        # check to see if an update to the miss-rate estimation is required
        if (self.access_cnt % self.mr_win == 0):
            self.update_mr()
        if (self.access_cnt % self.FN_mr_win == 0):
            self.update_FN_mr()
